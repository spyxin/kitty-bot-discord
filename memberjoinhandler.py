...
class MemberJoinHandler(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        bg.ellipse((835, 340), 250, 250, outline="lavenderblush", stroke_width=5)

        bg.text((960, 620), f"mreow~ welcome to {member.guild.name}!", color="palevioletred", font=font_big, align="center")
        bg.text((960, 740), f"{member.name} is member #{member.guild.member_count}!", color="palevioletred", font=font_small, align="center")

        img_file = discord.File(fp=bg.image_bytes, filename=randomized_image)

        await welcome_channel.send(f"*murrr~*, {member.mention} don't forget to read our rules and check out the available roles! ty for joing <3")
